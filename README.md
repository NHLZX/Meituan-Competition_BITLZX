...
![ROS](https://img.shields.io/badge/ROS-Noetic-green) ![Python](https://img.shields.io/badge/Python-3.8+-blue) ![C++](https://img.shields.io/badge/C++-14-purple) ![YOLOv8](https://img.shields.io/badge/YOLO-v8-yellow) ![Qwen](https://img.shields.io/badge/LLM-Qwen2.5--14B-blue)

本项目是针对**美团第三届低空经济智能飞行管理挑战赛（性能赛）**的完整开源解决方案。本方案基于 ROS 框架，结合了大模型 (Qwen) 的纯语义自然语言解析、YOLOv8 与 3D LiDAR 的多模态感知融合、鲁棒的有限状态机（FSM）决策、A* 与 DWA 局部避障规划，以及高精度的 ArUco 终点对齐技术，实现了地面小车在复杂仿真环境下的自主视觉语言导航。
...
* **Docker / LLM**: 平台要求使用 Docker 容器化部署，并需安装 Nvidia Container Toolkit 以支持 GPU 加速。需提前配置好 Ollama 环境以运行本地大模型。

---

## 🚀 核心解决方案 (Core Solution)

本方案采用**模块化**架构设计，将大模型的语义理解优势与传统控制算法的稳定性完美结合，极大提升了系统的实时性和鲁棒性。主要包含以下五个核心模块：

### 1. 基于千问大模型的语义解析引擎 (LLM Instruction Parser)
我们使用 Ollama 本地部署了强力大语言模型 **Qwen2.5:14b (千问)**，摒弃了传统的规则匹配，采用纯语义理解结合智能 JSON 拆包机制，将复杂的自然语言指令精准转化为下游状态机所需的可执行对：
* **结构化执行流提取**：通过严格的 Prompt 工程，限制模型输出 4 种动作与 8 种目标地标，强制转换为扁平的 `[Action, Marker, Action, Marker...]` JSON 数组。
* **基于认知逻辑的语义重构**：
  * **动作覆盖 (Turn Overrides Move)**：智能识别主导动作（例如 "Turn right and go to the tree" 大模型会直接理解为 `["right", "tree"]`，摒弃冗余的 `front`，防止小车过度前冲）。
  * **隐式动作推断 (Implicit Action)**：对于缺失运动方向的指令（如 "Stop at the bench"），大模型能结合物理世界常识，隐式补全为 `["front", "bench"]`。
  * **干扰物与绕行过滤 (Ignore Obstacles)**：利用大模型的长句理解能力，直接过滤掉无关实体与避障描述（如 "bypass pedestrians"），因为底层 DWA 规划器自带避障功能，将这些词过滤掉能极大提升状态机的鲁棒性。
* **智能 JSON 容错拆包**：编写了专用的拆包接口，兼容大模型可能输出的各类嵌套 JSON 格式（如带有 `"actions"` 或 `"steps"` 键的字典），确保数据被完美拍平并输送给导航节点。
...
* **🌟 核心创新：基于射线投射的 Mark 板定位**：
  针对地面平铺的终点 Mark 板，LiDAR 点云往往因掠射角问题而十分稀疏。我们针对类别 ID 9 (`mark`) 独立设计了**射线投射 (Ray-casting) 算法**。将相机光学中心的 3D 射线通过 TF 变换至全局 `map` 坐标系，并直接与地平面 (Z=0) 求交，从而计算出极其精准的地面 3D 坐标，解决了低角度下地面标志物测距不准的痛点。

### 3. 有限状态机 (FSM State Machine)
位于 `state_machine.cpp`，负责将大模型解析出的指令队列转化为物理动作：
...
### 4. 局部避障与路径规划系统 (A* + DWA)
为了应对比赛中随时可能出现的静态或动态障碍物，我们专门设计了高强度的局部规划器（位于 `localPlanner.cpp`, `dwaPlanner.cpp` 与 `localCostMap.cpp`）：
...
* **高精度坐标系转换**：检测到 ArUco 后，结合预设的相机外参（包含 0.314 弧度的俯仰角补偿），将坐标解算至 `base_link`，再通过 `tf2_ros` 映射至 `map` 全局坐标系，累积 5 帧平滑后锁定最终目标位置，并将其**优先级置为最高**，强制覆盖其他路径目标，确保小车完美停靠在 2m 半径内。
...
2. **拉取比赛基础 SDK 镜像**：
   ```bash
   docker pull [uav-challenge.tencentcloudcr.com/uav_challenge_2025/uav_challenge:sdk](https://uav-challenge.tencentcloudcr.com/uav_challenge_2025/uav_challenge:sdk)
   ```
...
4. **运行完整节点栈**：
   启动所有相关的感知、大模型解析与规控节点（请根据实际 launch 文件名进行调整）：
   ```bash
   roslaunch local_planner local_planner.launch
   python3 src/keyword_extractor/scripts/navigation_llm.py  # 启动大模型解析节点
   roslaunch yolo_detect yolo_detect.launch
   ```

5. **提交至评测平台**：
   将镜像打包，并通过官方 `submit.sh` 脚本提交测试任务：
   ```bash
   ./submit.sh submit [uav-challenge.tencentcloudcr.com/uav_challenge_2025/appkey:tag](https://uav-challenge.tencentcloudcr.com/uav_challenge_2025/appkey:tag) test.json 2
   ```
